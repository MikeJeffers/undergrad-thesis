...
public class tConnectionMatrix implements Runnable {
	...
	public tConnectionMatrix(thesisPointStructurev5 parent, tpointCloud ptCloud) {
		p = parent;
		pc = ptCloud;
	...
	public void run() {
		...
		this.genConnections();
		this.checkZValue();
		this.checkNodeOffset();
		this.checkForObstInt();
		this.removeMinNodes();
		this.checkAngles();
		this.checkConnectionIntersect();
		this.removeMinNodes();
		...
		if(runOnce){
			this.checkAnglesFinal();
			this.removeMinNodesFinal();
			p.gui.sendMessage("Last Angle Check");
			runOnce=false;
		}
	...
	private void checkAnglesFinal() {
		p.gui.sendMessage("Checking for angles under " + minAng
				* PConstants.RAD_TO_DEG + "*.");
		for (Vec3D pt : pc.getPoints())
			this.checkNodeAngleFinal(pt);
		p.gui.sendMessage("--Done checking angles.");
	}

	
	private void checkNodeAngleFinal(Vec3D pt) {
	...
	private void checkNodeOffset() {
		p.gui.sendMessage("Looking for connections that would make a Z.");
		Vector<connection> toHurt = new Vector<connection>();
		for (connection c : connections) {
			float xyDist = PApplet.dist(c.pts[0].x, c.pts[0].y, c.pts[1].x,
					c.pts[1].y);
			if (xyDist < 8.4f)
				toHurt.add(c);
		}

		for (connection c : toHurt)
			c.hurt(1);

		
		p.gui.sendMessage("--Found " + toHurt.size()
				+ " connections that would make a Z, hurting them for it.");
	}
	...
	private void removeMinNodesFinal() {
		int i = 0;
		p.gui.sendMessage("Removing nodes w/ less than 2  connections.");
		while (checkMinNodesFinal() > 0)
			i++;
		p.gui.sendMessage("--Nodes removed, took " + i + " runs.");
	}

	private int checkMinNodesFinal() {
		Vector<connection> toRemove = new Vector<connection>();
		for (Vec3D pt : this.pc.getPoints()) {
			Vector<connection> attached = new Vector<connection>();
			int n = 0;
	...
	private void checkZValue() {
		p.gui.sendMessage("Looking for max Z connections.");
		...
		for (Vec3D pt : this.pc.getPoints()) {
			...
			if (n > 0) {
				...
				int index = this.getMaxIndex(Zvals);
		...
		p.gui.sendMessage("--Helped " + toReinforce.size()
				+ " max Z connections");
	}

	private void checkConnectionIntersect() {
		p.gui.sendMessage("Looking for connections that would make a Z.");
		Vector<connection> toHurt = new Vector<connection>();
		...
		for (connection c : connections) {
			for (connection c2 : connections) {
				if (c != c2) {
					...
					Line3D l2 = c2.ln;
					float moveDist = 12f;
					Vec3D diff1 = l1.a.sub(l1.b);
					Vec3D diff2 = l1.b.sub(l1.a);
					diff1 = diff1.getNormalizedTo(moveDist);
					diff2 = diff2.getNormalizedTo(moveDist);
					Line3D nl1 = new Line3D(l1.a.sub(diff1), l1.b.sub(diff2));
					Vec3D diff3 = l2.a.sub(l2.b);
					Vec3D diff4 = l2.b.sub(l2.a);
					diff3 = diff3.getNormalizedTo(moveDist);
					diff4 = diff4.getNormalizedTo(moveDist);
					Line3D nl2 = new Line3D(l2.a.sub(diff3), l2.b.sub(diff4));

					LineIntersection interLine = nl1.closestLineTo(nl2);

					if (interLine.isIntersectionInside()) {
						if (interLine.getLength() < 6f) {
							toHurt.add(c);
							//break;
						}
					}
		...
		p.gui.sendMessage("--Flagged " + toHurt.size()
				+ " for intersecting the eachother.");
	}
	...
	Vector<Triangle3D> cullDupTri(Vector<Triangle3D> tri) {
		p.gui.sendMessage("Looking for duplicate triangles.");
		...
		PApplet.println("--" + getIndex.size() + " number of pairs found");
		...
		p.gui.sendMessage("--" + returnTri.size()
				+ " number of triangles remaining");
		return returnTri;
	}
