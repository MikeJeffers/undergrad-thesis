...
public class tConnectionMatrix implements Runnable {
	...
	public tConnectionMatrix(thesisPointStructurev3 parent, tpointCloud ptCloud) {
		p = parent;
		pc = ptCloud;
	...
	Vector<Triangle3D> cullDupTri(Vector<Triangle3D> tri) {
		...
		p.gui.sendMessage(getIndex.size() + " number of pairs found");
		...
		p.gui.sendMessage(returnTri.size() + " number of triangles remaining");
		return returnTri;
	}
